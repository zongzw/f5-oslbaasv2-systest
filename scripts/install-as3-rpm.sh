...
if [ $# -ne 3 ]; then
    echo "$0 <bigip-hostname> <username:password> <rpm-url>"
    exit 1
fi
...
if [ ! -f "$workdir/releases/$RPM_NAME" ]; then
    wget "$RPM_URL" -O $workdir/releases/$RPM_NAME
fi

TARGET_RPM="$workdir/releases/$RPM_NAME"
